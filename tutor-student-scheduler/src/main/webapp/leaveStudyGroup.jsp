...
<%  String reformat = request.getParameter("leave");
	String groupId = reformat.substring(10); 

	ResultSet rs = myUtil.getCourse(groupId);
	String test = null;
	while(rs.next()){
		test = rs.getString(1);
	}
%>
<form action = "leaveStudyGroupHandler.jsp" method = "get">
	<article style="height:200px;">
		<h1>- ARE YOU SURE? -</h1>
	</article>
	<article style="height: 390px;">
	<hr>
		<h4> <i><span style="font-size: 33px">You are about to leave: <br></span></i> <%=test + " (" + groupId + ")"%> <br><br> 
		<i>Do you really want to do this? You can always join this <br> group again if you decide to do so later. </i> </h4>
		<p> 
			<button id="Back" type="submit" formaction="listCurrentGroups.jsp" class="leave" style="top:350px;"> BACK </button>
		</p>
		<a>
			<button name="leave" id="leave" type="submit" value="<%=groupId %>" class="findMore" style="top:350px;"> LEAVE </button>
		</a>
	</article>
</form>
